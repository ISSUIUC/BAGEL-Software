...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_2|GPIO_PIN_7|GPIO_PIN_12|GPIO_PIN_11
                          |GPIO_PIN_0, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET);

  /*Configure GPIO pins : PA4 PA3 PA5 */
  GPIO_InitStruct.Pin = GPIO_PIN_4|GPIO_PIN_3|GPIO_PIN_5;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  /*Configure GPIO pins : PA2 PA7 PA12 PA11
                           PA0 */
  GPIO_InitStruct.Pin = GPIO_PIN_2|GPIO_PIN_7|GPIO_PIN_12|GPIO_PIN_11
                          |GPIO_PIN_0;
...
uint8_t I2C_SCAN(){
	...
	uint8_t start_msg[] = "I2C Scanning:\r\n";
	uint8_t end_msg[] = "Done\r\n";
	int len;

	uint8_t buffer[30];
	HAL_UART_Transmit(&huart1, start_msg, sizeof(start_msg), 1000);

	for (int i=0; i<128;i++){
		if (HAL_I2C_IsDeviceReady(&hi2c1, (uint16_t)(i<<1), 3, 5) == HAL_OK){
			len = sprintf((char*)buffer, "Detected 0x%X\r\n", i);
			HAL_UART_Transmit(&huart1, buffer, len, 1000);
		}
	}

	HAL_UART_Transmit(&huart1, end_msg, sizeof(end_msg), 1000);
	return 0;
}
